...
namespace J2JBreaker.Extensions
{
    ...
    {
        // IF-CHAIN:
        public static int DigitsIfChain(this long n)
        {
            if (n >= 0)
            {
                if (n < 10L) return 1;
        ...
        // USING LOG10:
        public static int DigitsLog10(this long n) =>
            n == 0L ? 1 : (n > 0L ? 1 : 2) + (int)Math.Log10(Math.Abs((double)n));

        // WHILE LOOP:
        public static int DigitsWhile(this long n)
        {
            int digits = n < 0 ? 2 : 1;
        ...
        // STRING CONVERSION:
        public static int DigitsString(this long n) =>
            n.ToString().Length;
    }
}
